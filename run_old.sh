#!/usr/bin/env bash
#
# Launch the GrandOrgue drone flute: fetch what is missing, build the organ,
# start GrandOrgue, open the control surface, and play. See ./run.sh for the
# browser-audio version, which needs none of this.
#
#   ./run_old.sh                      play forever with the GUI, contemplative
#   ./run_old.sh --mood restless      pick a mood
#   ./run_old.sh --seed 42            reproduce a performance exactly
#   ./run_old.sh --duration 60        stop after 60 seconds
#   ./run_old.sh --no-gui             player only, no web control surface
#   ./run_old.sh --http-port 9000     ask for a particular GUI port
#   ./run_old.sh --dry-run            no GrandOrgue, no audio, no GUI; print it
#   ./run_old.sh --rebuild            rebuild loops and organ from scratch
#
